...
LDPCCodec::LDPCCodec(CodecType type,
                     uint32_t packet_size):
    FECAbstractCodec(LDPCCodes,type),
    d_ptr(new LDPCCodecPrivateData)
{
...
void LDPCCodec::set_code_rate(const float &code_rate) noexcept
{
    d_ptr->set_code_rate(code_rate);
}

float LDPCCodec::get_code_rate()  noexcept
{
    return d_ptr->code_rate;
}

bool LDPCCodec::encode(void *data, uint32_t size) noexcept
{
    return d_ptr->encode(data,size);
}

bool LDPCCodec::decode(uint32_t src_pack_nb, uint32_t repair_pack_nb, uint32_t block_size, void **src_data, void **dst_data) noexcept
{
    return d_ptr->decode(src_pack_nb,repair_pack_nb,block_size,
                         src_data,dst_data);
}

uint32_t LDPCCodec::get_all_pack_nb() noexcept
{
    return d_ptr->total_pack_nb;
}

uint32_t LDPCCodec::get_repair_nb() noexcept
{
    return d_ptr->repair_pack_nb;
}

void **LDPCCodec::get_data() noexcept
{
